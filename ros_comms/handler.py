# your_robot_dashboard/ros_comms/handler.py
...
import time
import json
from .. import config # Use relative import to get config from parent directory
from ..callbacks import ros_message_callbacks # Import ROS message callbacks

# --- Global variables for ROS client, publishers, subscribers, state, and threads ---
ros_client = None
...
# --- ROS Subscriber Setup (Runs in its own thread) ---
def start_ros_subscriber_thread_internal():
    ...
    try:
        ...
        left_arm_sub.subscribe(lambda msg: ros_message_callbacks.left_arm_state_callback(msg, latest_joint_states)) # Pass latest_joint_states
        ...
        right_arm_sub.subscribe(lambda msg: ros_message_callbacks.right_arm_state_callback(msg, latest_joint_states)) # Pass latest_joint_states
        ...
        head_servo_sub.subscribe(lambda msg: ros_message_callbacks.head_servo_state_callback(msg, latest_joint_states)) # Pass latest_joint_states
...
# --- ROS Connection and Publisher Setup (Runs in its own thread) ---
def _ros_connect_thread_target():
    ...
    try:
        ...
        print(f"[Connect Thread] Calling local_ros_client.run() (assumed blocking)...")
        ...
        print("[Connect Thread] Publishers initialized.")

        ros_setup_done = False
        if subscriber_thread and subscriber_thread.is_alive():
            print("[Connect Thread] Warning: Old subscriber thread detected alive. It should terminate.")
...
# --- Helper function for safely terminating the ROS client ---
def safe_terminate_ros_client():
    global ros_client, ros_setup_done, left_arm_pub, right_arm_pub, head_servo_pub
