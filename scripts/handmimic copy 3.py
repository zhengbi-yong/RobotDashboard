...
async def main():
    ...
    try:
        ...
        async with websockets.connect(WEBSOCKET_URI) as websocket:
            ...
            print(f"--- 开始接收手势数据并控制机器人 (平滑系数: {SMOOTHING_FACTOR}) ---")
            ...
            async for message_str in websocket:
                ...
                if smoothed_angles is None:
                    # 如果是第一帧，直接使用当前计算值作为初始值
                    smoothed_angles = np.array(raw_target_angles, dtype=float)
                else:
                    # 使用指数移动平均 (EMA) 公式进行平滑
                    current_angles_np = np.array(raw_target_angles, dtype=float)
                    smoothed_angles = (SMOOTHING_FACTOR * current_angles_np) + \
                                      ((1.0 - SMOOTHING_FACTOR) * smoothed_angles)
                
                # 将平滑后的浮点数结果转换为整数列表，用于发送
                final_angles_to_send = [int(angle) for angle in smoothed_angles]

                # 4. 创建并发布包含平滑后角度的 ROS 消息
                ros_message = roslibpy.Message({'hand_angle': final_angles_to_send})
                ros_publisher.publish(ros_message)
                
                # 打印发送的指令，便于调试
                print(f"发送指令: {final_angles_to_send}", end='\r')
